...
#define DNS_RES_H_ERRNO(statp)	((statp)->res_h_errno)
...
#define res_nmkquery(statp, op, dname, class, type, data, datalen, \
		newrr, buf, buflen) \
	res_mkquery((op), (dname), (class), (type), (data), (datalen), \
		(newrr), (buf), (buflen))
#define res_nsearch(statp, dname, class, type, answer, anslen) \
	res_search((dname), (class), (type), (answer), (anslen))
#define res_nsend(statp, msg, msglen, answer, anslen) \
	res_send((msg), (msglen), (answer), (anslen))
#define DNS_RES_H_ERRNO(statp)	(h_errno)
#endif

#ifdef USE_SET_H_ERRNO
#define DNS_RES_SET_H_ERRNO(statp, err)	(set_h_errno(err))
#else
#define DNS_RES_SET_H_ERRNO(statp, err)	(DNS_RES_H_ERRNO(statp) = (err))
#endif
...
#define NO_MKQUERY_NEWRR        ((unsigned char *) 0)
    ...
    if ((len = res_nmkquery(&dns_res_state,
			    QUERY, name, class, type, NO_MKQUERY_DATA_BUF,
			    NO_MKQUERY_DATA_LEN, NO_MKQUERY_NEWRR,
			    msg_buf, sizeof(msg_buf))) < 0) {
	DNS_RES_SET_H_ERRNO(&dns_res_state, NO_RECOVERY);
	if (msg_verbose)
	    msg_info("res_nmkquery() failed");
	return (len);
    } else if ((len = res_nsend(&dns_res_state,
				msg_buf, len, answer, anslen)) < 0) {
	DNS_RES_SET_H_ERRNO(&dns_res_state, TRY_AGAIN);
	if (msg_verbose)
	...
	case NXDOMAIN:
	    DNS_RES_SET_H_ERRNO(&dns_res_state, HOST_NOT_FOUND);
	    break;
	case NOERROR:
	    if (reply_header->ancount != 0)
		DNS_RES_SET_H_ERRNO(&dns_res_state, 0);
	    else
		DNS_RES_SET_H_ERRNO(&dns_res_state, NO_DATA);
	    break;
	case SERVFAIL:
	    DNS_RES_SET_H_ERRNO(&dns_res_state, TRY_AGAIN);
	    break;
...
#define NOT_FOUND_H_ERRNO(he) ((he) == HOST_NOT_FOUND || (he) == NO_DATA)
    ...
    len = res_nsearch(&dns_res_state, name, class, type, answer, anslen);
    /* Begin API creep workaround. */
    if (len < 0 && DNS_RES_H_ERRNO(&dns_res_state) == 0) {
	DNS_RES_SET_H_ERRNO(&dns_res_state, TRY_AGAIN);
	...
	DNS_RES_SET_H_ERRNO(&dns_res_state, 0);
	       ...
	       && NOT_FOUND_H_ERRNO(DNS_RES_H_ERRNO(&dns_res_state))) {
...
#define SAVE_FLAGS (USER_FLAGS | XTRA_FLAGS)
	...
	if (DNS_RES_H_ERRNO(&dns_res_state) != 0) {
	    if (why)
		vstring_sprintf(why, "Host or domain name not found. "
				"Name service error for name=%s type=%s: %s",
				name, dns_strtype(type), 
				dns_strerror(DNS_RES_H_ERRNO(&dns_res_state)));
	    if (msg_verbose)
		msg_info("dns_query: %s (%s): %s",
			 name, dns_strtype(type), 
			dns_strerror(DNS_RES_H_ERRNO(&dns_res_state)));
	    switch (DNS_RES_H_ERRNO(&dns_res_state)) {
	    case NO_RECOVERY:
		return (DNS_FAIL);
	    case HOST_NOT_FOUND:
	    case NO_DATA:
	...
	msg_panic("dns_query: bad length %d (h_errno=%s)",
		  len, dns_strerror(DNS_RES_H_ERRNO(&dns_res_state)));
...
#define SAVE_HPREF_STATUS() do { \
	...
	if (why && status != DNS_OK) \
	    vstring_strcpy(hpref_rtext ? hpref_rtext : \
			   ...
			   vstring_str(why)); \
	hpref_h_errno = DNS_RES_H_ERRNO(&dns_res_state); \
    } while (0)
...
#define RESTORE_HPREF_STATUS() do { \
	...
	DNS_RES_SET_H_ERRNO(&dns_res_state, hpref_h_errno); \
    } while (0)
