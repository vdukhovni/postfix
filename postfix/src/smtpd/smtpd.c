...
#define BARE_LF_FLAG_REJECT	(1<<1)	/* Polite */
#define BARE_LF_FLAG_DISCONNECT	(1<<2)	/* Purist */

#define IS_BARE_LF_NORMALIZE(m)	((m) & BARE_LF_FLAG_NORMALIZE)
#define IS_BARE_LF_REJECT(m)	((m) & BARE_LF_FLAG_REJECT)
#define IS_BARE_LF_DISCONNECT(m) ((m) & BARE_LF_FLAG_DISCONNECT)

#define BARE_LF_FLAG_DISCONNECT_OR_REJECT \
	(BARE_LF_FLAG_DISCONNECT | BARE_LF_FLAG_REJECT)

#define IS_BARE_LF_DISCONNECT_OR_REJECT(m) \
	((m) & BARE_LF_FLAG_DISCONNECT_OR_REJECT)

static const NAME_CODE bare_lf_masks[] = {
    "normalize", BARE_LF_FLAG_NORMALIZE,
    "reject", BARE_LF_FLAG_REJECT,
    "disconnect", BARE_LF_FLAG_DISCONNECT,
    "yes", BARE_LF_FLAG_DISCONNECT,
    "no", 0,
    0, -1,				/* error */
};
				         ...
				         int out_error)
{
	...
	if (IS_BARE_LF_DISCONNECT_OR_REJECT(smtp_seen_bare_lf))
...
#define IS_SMTP_REJECT(s) \
	...
	state->error_mask |= MAIL_ERROR_PROTOCOL;
	log_whatsup(state, IS_BARE_LF_DISCONNECT(smtp_forbid_bare_lf) ?
		    "disconnect" : "reject", "bare <LF> received");
	smtpd_chat_reply(state, "%d 5.5.2 %s Error: bare <LF> received",
			 IS_BARE_LF_DISCONNECT(smtp_forbid_bare_lf) ?
			 521 : 550, var_myhostname);
