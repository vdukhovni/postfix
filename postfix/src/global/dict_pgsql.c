...
static void dict_pgsql_quote(DICT *, const char *, VSTRING *);
...
/* dict_pgsql_quote - escape a legacy expanded query parameter */
...
static void dict_pgsql_quote(DICT *dict, const char *name, VSTRING *result)
{
    ...
    const char *myname = "dict_pgsql_quote";
    size_t  len = strlen(name);
    size_t  buflen;
    int     err = 1;

    if (active_host == 0)
	msg_panic("%s: no active host for sql quoting", myname);

    if (len > (SSIZE_T_MAX - VSTRING_LEN(result) - 1) / 2)
	msg_panic("%s: unreasonable input length: %ld",
		  myname, (long) len);
    buflen = 2 * len + 1;
...
#define INIT_VSTR(buf, len) do { \
     ...
     * Legacy mode expands the whole query later, per connection, because SQL
     * quoting depends on the active connection.
     */
    if (dict_pgsql->statement_mode == PGSQL_STATEMENT_LEGACY) {
	if (!db_common_expand(dict_pgsql->ctx, dict_pgsql->query, name, 0,
			      query, 0))
	    return (0);
    } else if (!pgsql_stmt_bind(dict_pgsql, name)) {
	return (0);
    }
				        ...
				        VSTRING *query)
{
	...
	if (dict_pgsql->statement_mode == PGSQL_STATEMENT_LEGACY) {
	    ...
	    (void) db_common_expand(dict_pgsql->ctx, dict_pgsql->query,
	...
	if (dict_pgsql->statement_mode == PGSQL_STATEMENT_LEGACY) {
	    res = PQexec(host->db, vstring_str(query));
	} else if (dict_pgsql->statement_mode == PGSQL_STATEMENT_PARAMETERIZED) {
