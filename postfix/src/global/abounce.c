...
/* abounce_receive - resume pseudo thread after server reply event */
...
static void abounce_send(int event, void *context)
{
    ...
    if (event == EVENT_READ
	&& attr_scan(ap->fp, ATTR_FLAG_STRICT | ATTR_FLAG_MORE,
...
/* abounce_request - connect and suspend until the server replies */
			            ...
			            void *context)
{
	...
	if (attr_print(mp, ATTR_FLAG_MORE,
	    ...
	    || attr_print(mp, ATTR_FLAG_NONE,
			  ATTR_TYPE_END) != 0
	    || vstream_fclose(mp) != 0)
	    msg_panic("abounce_request: write request to memory stream: %m");
...
void    abounce_flush_verp(int flags, const char *queue, const char *id,
			           ...
			           void *context)
{
    abounce_request(MAIL_CLASS_PRIVATE, var_bounce_service,
			          ...
			          ABOUNCE_FN callback, void *context)
{
    ...
    abounce_request(MAIL_CLASS_PRIVATE, var_defer_service,
		    BOUNCE_CMD_VERP, flags, queue, id, encoding, smtputf8,
		    sender, dsn_envid, dsn_ret, verp, callback, context);
}
