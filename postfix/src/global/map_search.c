...
/*	const char *map_spec);
/* DESCRIPTION
/*	This module implements support for configurable
/*	searching of Postfix lookup tables.
/*
/*	map_search_init() must be called first.
/*
...
/*	Arguments:
/* .IP search_actions
/*	The mapping from search action string form to token form.
/*	These must be numbers in the range [0..126] (inclusive).
/*	The value MAP_SEARCH_CODE_UNKNOWN is reserved for the 'not
/*	found' result. The argument is copied (the pointer value,
/*	not the table).
/* .IP map_spec
...
#define MAP_SEARCH_CREATE_RETURN(x) do { \
	...
	if (search_list) vstring_free(search_list); \
	return (x); \
    } while (0)
	...
	while ((atom = mystrtok(&attr_value, CHARS_COMMA_SP)) != 0) {
	    ...
	    VSTRING_ADDCH(search_list, code);
	}
	VSTRING_TERMINATE(search_list);
    }
    ...
    if (search_list) {
	map_search->search_list = vstring_export(search_list);
	search_list = 0;
    } else {
	map_search->search_list = 0;
    }
...
int     main(int argc, char **argv)
{
	...
	const char *exp_search_list;	/* 0 or match */
    } TEST_CASE;
	...
	{"{type:name search=}", 1, "type:name", ""},
	{"{type:name search=one, two}", 0, 0, 0},
	{"{type:name {search=one, two}}", 1, "type:name", "\01\02"},
	{"{type:name {search=one, two, bad}}", 0, 0, 0},
	{0},
    };
    TEST_CASE *test_case;
	...
	if (!test_case->exp_return != !map_search_from_create) {
		...
		msg_warn("test case %d return expected %s actual {%s, %s}",
			 ...
			 map_search_from_create->search_list);
	    else
	...
	if (strcmp(string_or_null(test_case->exp_search_list),
		   string_or_null(map_search_from_create->search_list))) {
	    msg_warn("test case %d search_list expected=%s actual=%s",
		     (int) (test_case - test_cases),
		     string_or_null(test_case->exp_search_list),
		     string_or_null(map_search_from_create->search_list));
	    test_failed = 1;
	}
    }
