...
int     smtp_tls_trouble(SMTP_STATE *state, int protocol_stage)
{
	...
	if (tls->level > TLS_LEV_MAY)
	    msg_warn("%s: cleartext fallback, %s",
		     session->namaddrport,
		     (session->features & SMTP_FEATURE_STARTTLS) == 0 ?
		     "host did not offer STARTTLS" :
		     "our TLS engine is unavailable");
	break;
