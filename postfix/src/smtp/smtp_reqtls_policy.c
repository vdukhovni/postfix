...
/*	that will be matched in the specified order. A policy element
/*	must be an atom ("enforce", "best-effort", "disable", "error")
/*	or a type:table. A table lookup result must be an atom, not
/*	a type:table. To match a parent domain name with a table that
/*	wants and exact match, specify an explicit '.' before the parent
/*	domain name.
/*
...
/*	SMTP_REQTLS_POLICY_ACT_BEST_EFFORT (best-effort),
/*	SMTP_REQTLS_POLICY_ACT_DISABLE (disable), or
/*	SMTP_REQTLS_POLICY_ACT_ERROR (unknown command or database
/*	access error).
/*
				        ...
				        const char *nexthop_name)
{
	...
	if (STREQ(item, SMTP_REQTLS_POLICY_NAME_BEST_EFFORT)) {
	    return (SMTP_REQTLS_POLICY_ACT_BEST_EFFORT);
