...
int     bounce_recipient_dsn(VSTREAM *bounce, BOUNCE_INFO *bounce_info)
{
    ...
    post_mail_fputs(bounce, "");
    /* Fix 20140708: Don't send "utf-8" type in non-UTF8 message. */
	...
	/* Fix 20140708: Don't send UTF8 in non-UTF8 message. */
	if (bounce_info->smtputf8 || strncasecmp(rcpt->dsn_orcpt, "utf-8;", 6))
	    post_mail_fprintf(bounce, "Original-Recipient: %s", rcpt->dsn_orcpt);
    } else if (NON_NULL_EMPTY(rcpt->orig_addr)) {
	/* Fix 20140708: Don't send "utf-8" in non-UTF8 message. */
